...
class GamesControllerTest < ActionController::TestCase
  ...
  test "should roll dice" do
    ...
    assert_difference 'Game.find_by_id(@game.id).despair' do
      post :roll_dice, xhr: true, params: {:id => @game.id, :discipline => 0, :exhaustion => 0, :madness => 0, :pain => 6}
    end
  end
  ...
  test "should cast shadow" do
    ...
    assert_difference "Game.find_by_id(@game.id).despair", -1 do
      assert_difference "Game.find_by_id(@game.id).hope", +1 do
        put :cast_shadow, xhr: true, params: {:id => @game.id, :despair_coins => 1}
      end
    end
  end
  ...
  test "should shed light" do
    ...
    assert_difference "Game.find_by_id(@game.id).hope", -1 do
      put :shed_light, xhr: true, params: {:id => @game.id, :hope_coins => 1}
    end
  ...
  test "should invite players" do
    ...
    assert_difference "Play.count", +1 do
      assert_difference "Character.count", +1 do
        put :invite, xhr: true, params: {:id => @game.id, :email => users(:one).email}
      end
    end
  ...
  test "should uninvite players" do
    ...
    assert_difference "Play.count", -1 do
      assert_difference "Character.count", -1 do
        put :uninvite, xhr: true, params: {:id => @game.id, :email => users(:two).email}
        @game = Game.find_by_id(games(:one).id)
      end
    end
  ...
  test "should update game" do
    ...
    put :update, params: {id: @game, game: { despair: @game.despair + 1, hope: @game.hope + 1, name: "new name" }}
    assert_redirected_to game_path(assigns(:game))
    @game = Game.find_by_id(games(:one).id)
  ...
  test "should update game gm" do
    ...
    put :update, params: {id: @game, game: { gm_id: users(:two).id }}
    @game = Game.find_by_id(games(:one).id)
